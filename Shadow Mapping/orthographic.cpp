...
{
	bool success = vertexShader.load("shaders/orthographic/vertex.glsl") && fragmentShader.load("shaders/orthographic/fragment.glsl");
...
void Orthographic::draw(const Text &text) const
{
	const glm::mat4 modelMatrix = text.modelMatrix();
	glUniformMatrix4fv(uniform_modelMatrix, 1, GL_FALSE, &modelMatrix[0][0]);
